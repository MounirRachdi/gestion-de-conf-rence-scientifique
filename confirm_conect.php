...
if($type=='examinateur')
{
$_SESSION['logexam']=$login;

$_SESSION['pwdexam']=$password;
header("location:examinateur.php");
/*
...
if($num>0)
{ $_SESSION["exam"]=$log[0];
$query = "SELECT id_papier, nom, type, data FROM papier";
...
$ligne=mysql_fetch_row($res);
 ...
 { $_SESSION["auteur"]=$ligne[0];
 $_SESSION['ouvert']=true;
header("location:soumettre_papier.php");
}
else
...
header("location:liste_papier2.php");
}
else
echo "verfier vos information login ou mot de passe incorrect";
...
$login=$_POST['login'];
$password=$_POST['pwd'];
$_SESSION['loginresp']=$login;
$_SESSION['pawdresp']=$password;
//session_start(); //on démarre une session
	//$_SESSION['login'] = $login;
//session_register();
...
if($num>0)
echo "<script type='text/javascript'>window.open('admin/acceuil.php'); </script>";
else
